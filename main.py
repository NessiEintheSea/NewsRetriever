...
from src.config import validate, GENRES, ARTICLES_PER_GENRE
...
def run() -> None:
    ...
    logger.info(
        "Starting news agent  |  genres=%s  |  articles_per_genre=%d",
        GENRES,
        ARTICLES_PER_GENRE,
    )
    ...
    logger.info("Total articles fetched: %d", total_fetched)

    # ------------------------------------------------------------------ #
    # 2. Summarise (single API call for all articles)
    # ------------------------------------------------------------------ #
